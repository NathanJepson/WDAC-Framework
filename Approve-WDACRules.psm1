...
#NOTE: This function also adds File Publishers to the database once they are trusted or blocked
    ...
    try {
        ...
        $AppTrustLevels = Get-AppTrustedAllLevels -SHA256FlatHash $SHA256FlatHash -Driver:( ($AppInfo.SigningScenario -eq "Driver") -or ($AppInfo.SigningScenario -eq "DriverAndUserMode")) -UserMode:( ($AppInfo.SigningScenario -eq "UserMode") -or ($AppInfo.SigningScenario -eq "DriverAndUserMode")) -Connection $Connection -ErrorAction Stop
        if ($IsMSI) {
        ...
        #...The signing scenario therefore doesn't need to be selected if the app is blocked.
            ...
            #As far as I'm aware, rules for MSIs and Script files only live in usermode land
                ...
                #FilePath rules can only be applied to user-mode binaries
                    ...
                    } elseif ($SigningScenario -eq "DriverAndUserMode") {
                        $SigningLevelToTrustRuleAt = "Both"
                    } else {
                        $SigningLevelToTrustRuleAt = "UserMode"
                    }
                }
...
filter Approve-WDACRulesFilter {
    ...
    if (-not ($Level -or $Fallbacks)) {
        ...
        if (("" -ne $PreferredOrganizationRuleFallbacks) -and ($null -ne $PreferredOrganizationRuleFallbacks)) {
...
function Approve-WDACRules {
    ...
    begin {
        ...
        #Only grab preferred rule level and preferred fallbacks if both aren't supplied to this cmdlet
            ...
            if (("" -ne $TempLevel) -and ($null -ne $TempLevel)) {
                $Level = $TempLevel
            }
            if (("" -ne $TempFallbacks) -and ($null -ne $TempFallbacks)) {
...
Export-ModuleMember -Function Approve-WDACRulesFilter
Export-ModuleMember -Function Approve-WDACRules