...
function Get-WDACPolicySupplementalStatus {
    [cmdletbinding()]
    Param (
        [ValidateNotNullOrEmpty()]
        [Parameter(Mandatory=$true)]
    ...
    try {
        ...
        $Command = $Connection.CreateCommand()
        $Command.Commandtext = "Select BaseOrSupplemental from policies WHERE PolicyGUID = @PolicyGUID"
        $Command.Parameters.AddWithValue("PolicyGUID",$PolicyGUID) | Out-Null
        $Command.CommandType = [System.Data.CommandType]::Text
        $Reader = $Command.ExecuteReader()
        $Reader.GetValues() | Out-Null
        while($Reader.HasRows) {
            if($Reader.Read()) {
                $result = [bool]$Reader["BaseOrSupplemental"]
                break
            }
        }
        $Reader.Close()
...
function Get-WDACPolicyGUIDGivenName {
    [cmdletbinding()]
    Param (
        [ValidateNotNullOrEmpty()]
        [Parameter(Mandatory=$true)]
    ...
    try {
        ...
        $Command = $Connection.CreateCommand()
        $Command.Commandtext = "Select PolicyGUID from policies WHERE PolicyName = @PolicyName"
        $Command.Parameters.AddWithValue("PolicyName",$PolicyName) | Out-Null
        $Command.CommandType = [System.Data.CommandType]::Text
        $Reader = $Command.ExecuteReader()
        $Reader.GetValues() | Out-Null
...
function New-WDACFilePublisherByCriteria {
    ...
    try {
        ...
        if ($IsBlocking) {
            ...
            $MaxVersionNumber = Get-MaxFileVersionPrompt -Prompt "What MaximumVersionNumber would you like to set for this file publisher block rule?" -FileVersionInfo "FileVersion for this app is $CurrentVersionNum .`n FilePublisher info: FileName $FileName and Publisher Common Name: $($ThePublisher.LeafCertCN) and PcaCertTBSHash: $($ThePublisher.PcaCertTBSHash)" -CurrentVersionNum $CurrentVersionNum -MinimumVersion $NewVersionNumber
            
            if ( (Get-WDACFilePublishersDefinitive -PublisherIndex $PublisherIndex -FileName $FileName -SpecificFileNameLevel $SpecificFileNameLevel -MinimumAllowedVersion $NewVersionNumber -Connection $Connection -ErrorAction SilentlyContinue).Count -ge 1) {
                throw "A file publisher with these properties and minimum version number $NewVersionNumber already exists in the database."
            }
            Add-WDACFilePublisher -PublisherIndex $PublisherIndex -FileName $FileName -BlockingPolicyID $PolicyID -Comment $Comment -MinimumAllowedVersion $NewVersionNumber -MaximumAllowedVersion $MaxVersionNumber -SpecificFileNameLevel $SpecificFileNameLevel -Connection $Connection -ErrorAction Stop | Out-Null
