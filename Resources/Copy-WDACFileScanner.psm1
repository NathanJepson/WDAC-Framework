function Copy-WDACFileScanner {
    ...
    $Result = Invoke-Command -ErrorAction SilentlyContinue -Session $sess -ScriptBlock {
        $IsModulePresent = Test-Path "$($Env:Programfiles)\WindowsPowerShell\Modules\WDACFileScanner\WDACFileScanner.psm1"
    ...
    $Result | ForEach-Object {
        ...
        #Case: Module WDACAuditing.psm1 is not yet installed on the remote machine
            ...
            #Note: PowerShell remoting will fail if a non-audit WDAC policy is on the remote machine (due to Constrained Language Mode)
                ...
                try {
                    Copy-Item -Path (Join-Path $PSModuleRoot -ChildPath $ModulePath) -Destination "\\$PSComputerName\$TempDrive`$\Program Files\WindowsPowerShell\Modules\WDACFileScanner\" -ErrorAction Stop
                } catch {
                    try {
                        $sess = New-PSSession -ComputerName $PSComputerName -ErrorAction Stop; 
                        Copy-Item -ToSession $sess -Path (Join-Path $PSModuleRoot -ChildPath $ModulePath) -Destination "$TempDrive`:\Program Files\WindowsPowerShell\Modules\WDACFileScanner\" -ErrorAction Stop
                        $sess | Remove-PSSession
                    } catch {
                        #FIXME / TODO
            ...
            $jobs += (Start-Job -Name ("CopyWDACFileScanner_" + $_.PSComputerName) -ScriptBlock $ScriptBlock -ArgumentList $_.PSComputerName,$TempDrive,$PSModuleRoot,$ModulePath)
            $iterator += 1;
        }
    }
